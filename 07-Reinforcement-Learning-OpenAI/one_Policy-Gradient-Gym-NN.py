...
outputs = tf.nn.tanh(logits)  # probability of action 0 (left)

probabilties = tf.concat(axis=1, values=[outputs])
action = probabilties
...
with tf.Session() as sess:
    ...
    for iteration in range(num_iterations):
        ...
        for game in range(num_game_rounds):
            ...
            for step in range(max_game_steps):
                ...
                print(action_val,action_val[0])
                # Perform Action
                observations, reward, done, info = env.step(action_val[0])
    ...
    meta_graph_def = tf.train.export_meta_graph(filename='./models/omy-650-step-model.meta')
    saver.save(sess, './models/omy-650-step-model')
...
with tf.Session() as sess:
    ...
    new_saver = tf.train.import_meta_graph('./models/omy-650-step-model.meta')
    new_saver.restore(sess,'./models/omy-650-step-model')
